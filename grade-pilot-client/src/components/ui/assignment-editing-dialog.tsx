...
interface AssignmentEditingDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onUpdate: (updatedAssignment: AssignmentType) => void;
...
export default function AssignmentEditingDialog({
  ...
  const updateAssignment = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    ...
    if (assignmentMark && currentCompletionStatus == 'Complete') {
